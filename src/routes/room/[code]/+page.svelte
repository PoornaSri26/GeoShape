<script lang="ts">
	...
	import { playRoundStart, playGameOver, playJoin, playLeave, unlockAudio } from '$lib/sound';
	import { missedDeck } from '$lib/stores/missedDeck.svelte';

	// Sound: player count change (join/leave)
	let _prevPlayerCount = -1;
	$effect(() => {
		const count = room?.players.length ?? 0;
		if (_prevPlayerCount !== -1 && count !== _prevPlayerCount) {
			if (count > _prevPlayerCount) playJoin();
			else playLeave();
		}
		_prevPlayerCount = count;
	});

	// Auto-record missed shape into Missed Deck when round ends unsolved
	let _prevRoundEndAt = 0;
	$effect(() => {
		const result = game.roundResult;
		if (result && result.nextRoundAt !== _prevRoundEndAt) {
			_prevRoundEndAt = result.nextRoundAt;
			if (me && !me.solved) {
				const catId = result.categoryId ?? game.round?.categoryId ?? 8;
				const shapeId = result.shapeId ?? 0;
				missedDeck.addMissed({
					categoryId: catId,
					shapeId,
					name: result.answer,
					nameDe: result.answerDe,
					path: result.path || game.round?.path || '',
					viewBox: result.viewBox || game.round?.viewBox || '0 0 1000 1000',
					answers: result.answers ?? [result.answer],
					info: result.info,
	...
	$effect(() => {
		...
		if (over && !_prevGameOver) playGameOver();
		_prevGameOver = over;
	});
