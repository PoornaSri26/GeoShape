<script lang="ts">
	...
	let _lastWarningSec = -1;
	$effect(() => {
		if (!revealed && !paused && secondsLeft > 0 && secondsLeft <= 5 && secondsLeft !== _lastWarningSec) {
			_lastWarningSec = secondsLeft;
			playWarning();
		}
	});
...
<div class="flex h-full min-h-0 flex-col gap-2">
		...
		<div class="flex items-center gap-2">
			...
			{#if !revealed}
				<button
					...
					class="rounded border border-border bg-secondary px-2 py-0.5 text-xs font-black transition-transform hover:scale-105"
					title="Toggle Hint Clue"
				>
					💡 Hint
				</button>
				<button
					type="button"
					onclick={() => (negativeSpace = !negativeSpace)}
					class="rounded border border-border px-2 py-0.5 text-xs font-black transition-transform hover:scale-105 {negativeSpace ? 'bg-main text-ink' : 'bg-surface text-ink/70'}"
					title="Toggle Negative Space Mode (View as Hole in Map)"
				>
					🕳️ Negative
				</button>
			{/if}
	...
	{#if showHint && !revealed}
		<div class="flex flex-col gap-1 rounded-base border-2 border-border bg-secondary/40 px-3 py-2 text-xs font-bold text-center animate-pulse">
			<div>
				💡 Hint: Starts with <span class="font-black text-sm text-main-accent uppercase">"{round.hintFirstChar ?? '?'}"</span> ({round.hintLength ?? '?'} letters long)
			</div>
			{#if round.capitalName}
				<div class="text-[11px] text-ink/70 font-semibold">
					🏙️ Capital City: <span class="font-black text-ink">{round.capitalName}</span> (+50 Bonus)
				</div>
			{/if}
		</div>
	{/if}
	...
	>
		{#key round.round}
			<svg {viewBox} preserveAspectRatio="xMidYMid meet" class="h-full w-full">
				<defs>
					<mask id="neg-space-mask-{round.round}">
						<rect x="-2000" y="-2000" width="6000" height="6000" fill="white" />
						<path d={targetPath} fill="black" />
					</mask>
				</defs>

				{#if negativeSpace && !revealed}
					<!-- Negative space background block with target cutout -->
					<rect
						x="-2000"
						y="-2000"
						width="6000"
						...
						mask="url(#neg-space-mask-{round.round})"
					/>
				{/if}
