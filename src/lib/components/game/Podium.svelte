<script lang="ts">
	...
	function getBadge(score: number): string {
		if (score >= 500) return '🏆 Geography Guru';
		if (score >= 250) return '⭐ Shape Master';
		if (score >= 100) return '🌍 Globe Trotter';
		return '🌱 Rookie';
	}
	...
	{#each slots as s, i (i)}
		{#if s.player}
			<div class="flex flex-1 flex-col items-center gap-1.5">
				{#if s.place === 1}
					<span class="text-2xl leading-none animate-bounce">👑</span>
				{/if}
				<Avatar
				...
				/>
				<span
					class="max-w-full truncate text-center font-black {s.place === 1 ? 'text-base' : 'text-sm'}
						{s.player.id === playerId ? 'text-main-accent' : ''}"
				>
					{s.player.name}
				</span>
				<span class="rounded border border-border bg-surface px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm">
					{getBadge(s.player.score)}
				</span>
				<div
