		...
		<div class="flex flex-col gap-1.5">
			...
			<div class="flex gap-2">
				{#each [{ on: true, label: t('common.on') }, { on: false, label: t('common.off') }] as opt (opt.on)}
					<button
						type="button"
						class="flex-1 rounded-base border-2 border-border px-3 py-1.5 text-sm font-extrabold transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none {settings.dark ===
						opt.on
							? 'bg-main shadow-shadow'
							: 'bg-surface'}"
						onclick={() => settings.setDark(opt.on)}
					>
						{opt.label}
					</button>
				{/each}
