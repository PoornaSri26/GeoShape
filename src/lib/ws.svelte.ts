...
class GameSocket {
		...
		switch (msg.type) {
			...
			case ServerMsg.ROUND_START:
				...
				this.round = {
					...
					hintFirstChar: msg.hintFirstChar,
					hintLength: msg.hintLength,
					capitalName: msg.capitalName
				};
				break;
			...
			default:
				...
				this.errorCode = 'unknown_message';
				this.#pendingAck?.reject(new Error(this.error));
				this.#pendingAck = null;
				break;
		}
	}

	/** Resets only game/room play state; keeps the WebSocket open. */
	#clearRoomState(): void {
		this.room = null;
		this.round = null;
		this.roundResult = null;
	...
	/** Full reset: clears all state and closes the WebSocket. */
	#resetGame(): void {
		this.#clearRoomState();
		this.playerId = null;
		this.connected = false;
	...
	/** Alias used by the home page. */
	getStats(clientId: string): void {
		this.requestStats(clientId);
	}

	dismissGameOver(): void {
		this.gameOver = null;
		// Note: we don't reset other state here; just dismiss overlay.
	}
