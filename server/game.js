...
export function syncJoiner(room, player) {
	...
	if (room.roundActive && room.currentShape) {
		...
		const primaryAnswer = room.currentShape.answers[0] ?? room.currentShape.name;
		const capitalName = room.currentShape.info?.capital ?? null;
		send(player, {
			...
			hintFirstChar: primaryAnswer.charAt(0).toUpperCase(),
			hintLength: primaryAnswer.length,
			capitalName
		});
		if (room.paused) {
...
function startRound(room) {
	...
	const category = getCategory(room.categoryId);
	const primaryAnswer = shape.answers[0] ?? shape.name;
	const capitalName = shape.info?.capital ?? null;
	roomManager.broadcast(room, {
		type: ServerMsg.ROUND_START,
		round: room.round,
		maxRounds: room.maxRounds,
...
function endRound(room) {
	...
	roomManager.broadcast(room, {
		...
		shapeId: room.currentShape?.id ?? 0,
		categoryId: room.categoryId,
		viewBox: category?.viewBox ?? '0 0 1000 1000',
		path: room.currentShape?.path ?? room.roundPath ?? '',
		answers: room.currentShape?.answers ?? [],
		capital: room.currentShape?.capital ?? null,
		answer,
		answerDe,
		info,
		context,
